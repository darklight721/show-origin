...
export default class {
  ...
  openResultEditor() {
    ...
    let resultFileName = `${this.title}:${this.row}.origin`;
    return atom.workspace.open(resultFileName);
  }

  highlightLine(editor) {
    if (!this.projectPath || !this.path || !this.line) return;

    let path = escapeRegExp(relative(this.projectPath, this.path));
    let line = escapeRegExp(this.line.trim());
    let term = String.raw`^diff --git a\/${path}[\s\S]*?\+\s*?${line}`;
